...
def join_path(path1: str, path2: str) -> str:
    """Safely join two USD stage paths into a valid absolute path."""
    ...
    if not path2.startswith("/"):
        path2 = "/" + path2
    # Remove trailing slash in path1 if present
    if path1.endswith("/"):
        path1 = path1[:-1]
    return path1 + path2
...
class UsdHelper:
    ...
    def add_subroot(self, root="/world", sub_root="/obstacles", pose: Optional[Pose] = None):
        if sub_root is None or sub_root.strip() == "":
            sub_root = "obstacles"
        full_path = join_path(root, sub_root)
        xform = self.stage.DefinePrim(full_path, "Xform")
        if pose is not None:
    ...
    def add_world_to_stage(
        ...
        if obstacles_frame is None or obstacles_frame.strip() == "":
            obstacles_frame = "obstacles"
    ...
    def add_mesh_to_stage(
        ...
        # 检查是否已存在
        existing_prim = self.stage.GetPrimAtPath(root_path)
