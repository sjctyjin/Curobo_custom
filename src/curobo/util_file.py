...
def join_path(path1: str, path2):
    # 如果 path2 已经是个 dict，就直接 return，让 load_yaml(dict) 走原路返回它自己
    if not isinstance(path2, str):
        return path2
    
    # 本地 FS 路径判断：Windows 下有反斜杠或驱动器标识
    if "\\" in path1 or (os.name == "nt" and ":" in path1):
        # 遇到多余的斜杠也让 os.path.join 去处理
        return os.path.join(path1, path2)

    # 否则当成 USD prim 路径
    # 先删掉多余的 '/'
    p1 = path1.strip("/")
    p2 = path2.strip("/") if isinstance(path2, str) else str(path2)
    # 拼成 '/a/b'
    return "/" + "/".join([p1, p2])
    ...
    """
    USD prim 路径拼接，始终使用正斜杠，并保证返回一个绝对 POSIX 样式路径。
    
    # 只保留非空字符串片段，并去掉首尾所有斜杠
    parts = [p.strip("/") for p in paths if isinstance(p, str) and p.strip("/")]
    # 用 '/' 连接，并在最前面加上 '/'
    return "/" + "/".join(parts)
    """
